...
class CategoryRepositoryImpl(CategoryRepository):
    ...
    def get_by_name(self, name: str) -> Category | None:
        try:
            ...
            logger.error(f"Database error getting category with name '{name}': {e}")
            raise

    def create(self, category: Category) -> Category:
        try:
            model = CategoryMapper.from_domain(category)
            self.db.add(model)
            self.db.commit()
    ...
    def update(self, category: Category) -> Category | None:
        try:
            ...
            if not model:
                return None
            CategoryMapper.update_model_from_domain(model, category)
            self.db.commit()
            self.db.refresh(model)
            return CategoryMapper.to_domain(model)

        except SQLAlchemyError as e:
            logger.error(f"Error updating category with ID {category.id}: {e}")
            self.db.rollback()
            raise
